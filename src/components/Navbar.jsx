...
const Navbar = ({ setArticles }) => {
    ...
    return (
        <div className='fixed w-full  dark:bg-blue-900 z-10 shadow-lg bg-emerald-300'>
            <div className='max-w-7xl mx-auto px-4 py-3 flex items-center justify-between'>
                ...
                <div className='hidden md:flex space-x-6'>
                    {
                        links.map((link) => {
                            return <Link to={`/${link.toLowerCase()}`} key={link} className='text-gray-900 dark:text-gray-200 dark:hover:text-white hover:text-blue-600 transition'>
                                {link}
                            </Link>
                        })
                    }
                    ...
                    <div className='md:hidden px-4 pb-4'>
                        {
                            links.map((link) => {
                               return <Link key={link}
                                    ...
                                    className='block py-2 text-gray-900 dark:text-gray-200 dark:hover:text-white hover:text-blue-600 transition'
                                >
                                    {link}
                                </Link>
                            })
